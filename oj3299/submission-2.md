...
```text
ข้อนี้นับ inputมาสองตัวครับ คือจํานวนการเจิบโตว่าแต่ละแถบจะยาวเท่าไหน่ในแนวตั้ง และจะสิ้นสุดที่ล่องที่เท่าไหร่ครับ ที่นี้เราน้องหาว่ามันจะไปจบที่แถบไหน
```
...
```text
ผมสังเกตุว่าเราสามารถใบ้สูตรนี้คํานวนได้ทันทีครับคือ N = m.ceil((m.sqrt(8*N + 1) - 1) / 2) มาจาก สูตรจำนวนสามเหลี่ยมครับคือ เป็นการแปลงค่า N ให้เป็น ลำดับของแนวทแยง ถ้าแบบที่แถบมีขนาดเป็น 1 โดยใช้สูตรแก้สมการผลรวมอนุกรมเลขคณิตครับสูตรมันคือ k(k+1)/2 น้อยกว่า nเท่าไหร่ในแนวแทยง จากนั้นนำลำดับแนวทแยงที่คำนวณได้มาหารด้วยขนาดแถบจริง แล้วใช้ฟังก์ชัน ceil เพื่อปัดเศษขึ้น ทำให้ทราบว่าตกอยู่ในแถบกลุ่มที่เท่าไหร่ครับ
```
...
```text
ตามแผนแนกครับผมสังเกตุว่าเราสามารถใบ้สูตรนี้คํานวนได้ทันทีครับคือ N = m.ceil((m.sqrt(8*N + 1) - 1) / 2) มาจาก สูตรจำนวนสามเหลี่ยมครับคือ เป็นการแปลงค่า N ให้เป็น ลำดับของแนวทแยง ถ้าแบบที่แถบมีขนาดเป็น 1 โดยใช้สูตรแก้สมการผลรวมอนุกรมเลขคณิตครับสูตรมันคือ k(k+1)/2 น้อยกว่า nเท่าไหร่ในแนวแทยง จากนั้นนำลำดับแนวทแยงที่คำนวณได้มาหารด้วยขนาดแถบจริง แล้วใช้ฟังก์ชัน ceil เพื่อปัดเศษขึ้น ทำให้ทราบว่าตกอยู่ในแถบกลุ่มที่เท่าไหร่ครับ
```
...
```text
กรณีพอดีกับจุดสิ้นสุดของแนวทแยง
```
