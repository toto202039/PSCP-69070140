...
```text
รับinputมา1ตัวครับเป็นเลขสองเลข ทีนี้เราจะหาว่าในช่วงนี้มีเลขจํานวนเฉพาะเท่าไหร่
...
```text
ผมจะใช้ while ลูปสร้างลิสต์ data เก็บตัวเลขตั้งแต่ตัวแรกถึงตัวที่สองโดยหลักการหาจำนวนเฉพาะคือเลขที่มากกว่า 1 และไม่มีตัวเลขไหนหารมันลงตัวเลยนอกจาก 1 ผมเลยif not item % i ถ้ามันจะตัวไหนที่หารลงตัวก็จะหยุดทันทีและไม่บวกอะไรเพราะถ้่ามีตัวหารลงตัวมันก็ไม่ใช่จํานวนเฉพาะครับ และก็appendตัวที่ไม่มีตัวหารมันลงไปในlistและก็prit lenออกมา และpritทุกตัวในlistออกมา
```
...
```text
เหมือนวิธีแรกครับผมจะใช้ while ลูปสร้างลิสต์ data เก็บตัวเลขตั้งแต่ตัวแรกถึงตัวที่สองโดยหลักการหาจำนวนเฉพาะคือเลขที่มากกว่า 1 และไม่มีตัวเลขไหนหารมันลงตัวเลยนอกจาก 1 ผมเลยif not item % i ถ้ามันจะตัวไหนที่หารลงตัวก็จะหยุดทันทีและไม่บวกอะไรเพราะถ้่ามีตัวหารลงตัวมันก็ไม่ใช่จํานวนเฉพาะครับ และก็appendตัวที่ไม่มีตัวหารมันลงไปในlistและก็prit lenออกมา และpritทุกตัวในlistออกมา
```
...
```text
ทดสอบว่าโปรแกรมจัดการกับตัวเลขที่น้อยกว่าหรือเท่ากับ 1 ได้ถูกไหม
```
...
```text
ทดสอบการทำงานของลูป range(2, item) เพราะเมื่อเจอเลข 2 ที่ป็นเคสพิเศษของจำนวนเฉพาะที่เป็นเลขคู่ อาจผิดพลาดได้
```
